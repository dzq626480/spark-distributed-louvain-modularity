...
object LouvainCore {
  ...
  def createLouvainGraph[VD: ClassTag](graph: Graph[VD,Long]) : Graph[VertexState,Long]= {
    ...
    val louvainGraph = graph.outerJoinVertices(nodeWeights)((vid, _, weightOption)=> {
   ...
   */
  private def sendMsg(e: EdgeContext[VertexState, Long, Map[(Long, Long), Long]]): Unit = {
    e.sendToDst(Map((e.srcAttr.community, e.srcAttr.communitySigmaTot) -> e.attr))
  }
